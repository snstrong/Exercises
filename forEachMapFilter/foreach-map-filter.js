...
function addKeyAndValue(arr,key,value){
    const keyAndValueAdded = arr.map(function(val) {
        val [key] = value;
        return val;
    })
...
function vowelCount(str){
    ...
    vowels.forEach(function(vowel) {
        ...
        if (count != 0) obj [vowel] = count; 
    });
    return obj;

    // for (let vowel of vowels) {
    //     let count = 0;
    //     for (let char of justVowels) {   
    //         if (char === vowel) {
    //             count++;
    //         }
    //     }
...
function extractFullName(arr){
    return arr.map(function(val) {
        let fullName = "";
        return fullName = `${val['first']} ${val['last']}`;
    })
}
...
function find(arr, searchValue) {
    const filtered = arr.filter(function(val, idx) {
...
function removeVowels(str) {
    const lowerCased = Array.from(str.toLowerCase());
    let noVowels = lowerCased.filter(function(char) {
        return !isVowel(char);
    });
